...
class InstaBot:
    ...
    def get_media_id_by_tag(self, tag):
        ...
        if self.login_status:
            if tag.startswith('l:'):
                ...
                log_string = "Searching for user: %s" % (tag)
                ...
                for d in self.media_by_tag:
                    ...
                    if media_size > 0 or media_size < 0:
                        ...
                        if ((l_c <= self.media_max_like and
                            ...
                            try:
                                if self.by_location:
                                    ncaption = len(self.media_by_tag[i]['caption'])
                                else:
                                    ncaption = len(self.media_by_tag[i]['node']['edge_media_to_caption']['edges'])

                                if (ncaption > 1):
                                    if self.by_location:
                                        caption = self.media_by_tag[i]['caption'].encode(
                                            'ascii', errors='ignore')
                                    else:
    ...
    def new_auto_mod_follow(self):
        ...
        if time.time() > self.next_iteration["Follow"] and \
            ...
            if self.by_location:
                ccodeuserid = self.media_by_tag[0]["owner"]["id"]
            else:
            ...
            if check_already_followed(self, user_id=ccodeuserid) == 1:
                self.write_log("Already followed before " + ccodeuserid)
